...
export const caseStudies: CaseStudy[] = [
  ...
  },
  {
    ...
    "role": "Proposition spontanée conçue et développée de bout en bout chez African Digit Consulting : modèle de données Convex, moteur de disponibilité temps réel, workflow de réservation et de validation, design system de marque clair et chaleureux à accent ocre, photo-forward, avec barre de réservation type hôtel.",
    "year": "2026",
    "status": "dev",
    "stack": [
      "TanStack Start v1.121",
    ...
    "architectureSummary": "L'application est un monolithe SSR TanStack Start (React 19, Vite 7) déployé sur Vercel, adossé à un backend unique Convex qui sert à la fois de base de données et de couche temps réel. Le modèle métier tient en quatre tables : studios (catégorie, prix à la nuitée, équipements, photos, ordre d'affichage), reservations (client, dates d'arrivée et de départ, nombre de nuits, prix total, statut pending / confirmed / refused / cancelled), blockedPeriods (indisponibilités commerciales hors réservation : entretien, mise hors-ligne) et admins. La pièce centrale est une query de disponibilité, checkAvailability, qui détecte tout chevauchement de dates pour un studio donné via la condition d'intersection d'intervalles (checkIn < checkOut demandé ET checkOut > checkIn demandé), en ignorant les réservations refusées ou annulées et en croisant les périodes bloquées ; getBookedDates alimente un calendrier côté client. Les abonnements temps réel de Convex propagent instantanément à tous les visiteurs la prise d'un créneau ou la confirmation d'une réservation par l'admin, sans rafraîchissement. Better Auth est réservé à l'authentification de l'administrateur (validation des demandes, blocage de périodes) : la réservation publique reste sans compte, pour ne pas ajouter de friction à la conversion. Au-dessus de ce socle, le site public adopte un langage de marque clair, chaleureux et honnête, résolument photo-forward : un seul accent, un ocre doux (#B8863B), posé sur un monochrome espresso / crème, avec Fraunces en titrage et Hanken Grotesk en texte. Le hero est responsive : sur desktop, la photo vit dans un panneau aux coins arrondis, contenue et nette, à côté du texte sur fond clair, surmonté d'une barre de réservation pleine largeur ; sur mobile, la photo passe en plein écran avec le texte en surimpression, un slideshow enchaînant les vues en fondu avec un léger ken-burns. La pièce de conversion est une barre de réservation type hôtel (champ Séjour ouvrant un calendrier de plage maison, menus Voyageurs et Résidence construits sur mesure avec Radix UI Select et Popover, sans aucun contrôle natif) qui mène à la fiche de la résidence avec les dates pré-remplies, doublée sur mobile d'un CTA de réservation collant. La navbar est adaptative (pilule espresso flottante au scroll, texte sombre sur le hero clair en desktop, clair sur l'image en mobile, menu plein écran sur mobile) et une galerie photo s'ouvre en lightbox plein écran via un portail React. La couche de mouvement GSAP reste sobre et fail-safe : SplitText révèle les titres ligne par ligne derrière un masque, des primitives Reveal et Stagger animent l'apparition des sections et la cascade des cartes, et ImageReveal dévoile les photos par masque clip-path, chaque effet partant d'un état de base visible pour rester lisible sans JavaScript et sous prefers-reduced-motion.",
    "architectureDiagram": {
      "nodes": [
        {
          "id": "visitor",
    ...
    "decisions": [
      ...
      },
      {
        ...
        "choice": "Centraliser toute l'animation dans un module de primitives (SplitText pour la révélation des titres ligne par ligne derrière un masque, Reveal, Stagger, ImageReveal en clip-path) sur ScrollTrigger, où chaque effet anime uniquement transform, opacity ou clip-path à partir d'un état de base déjà visible (fromTo avec immediateRender:false), au lieu de cacher le contenu en attendant le scroll.",
        "rationale": "Sur des connexions mobiles contraintes et en SSR, un contenu masqué par défaut qui dépend d'un trigger JavaScript est un risque d'écran vide ; partir d'un état visible garantit que la page reste lisible sans JavaScript, sous prefers-reduced-motion, et même si un ScrollTrigger ne se déclenche pas. Animer seulement transform, opacity et clip-path garde l'effet sur le compositeur, sans reflow.",
        "tradeoff": "Le module ajoute une dépendance (GSAP, dont le plugin SplitText) et de la discipline d'intégration (retirer toute classe .reveal du markup interne pour éviter la double animation) là où de simples transitions CSS auraient suffi ; en échange, le mouvement est cohérent, testable et toujours dégradable proprement."
      },
      {
        "title": "Barre de réservation type hôtel avec composants déroulants sur mesure plutôt que des contrôles natifs",
        "choice": "Faire de la barre de réservation type hôtel (champ Séjour ouvrant un calendrier de plage maison, menus Voyageurs et Résidence construits sur Radix UI Select et Popover) la pièce maîtresse du hero, qui mène à la fiche de la résidence avec les dates déjà pré-remplies, doublée d'un CTA collant sur mobile, et bannir tout contrôle natif (select, input date) au profit de composants entièrement personnalisés.",
        "rationale": "Les contrôles natifs sont jugés peu esthétiques, incohérents entre navigateurs et impossibles à aligner sur la charte ; reprendre les codes de réservation que la clientèle connaît déjà (Booking, Airbnb) place la conversion au premier plan dès le hero, et Radix apporte l'accessibilité (clavier, ARIA, focus) sans imposer son style, ce qui laisse la barre s'habiller en ocre / espresso. Pré-remplir les dates dans le calendrier de la fiche supprime une saisie redondante et raccourcit le chemin vers la demande.",
        "tradeoff": "Construire un calendrier de plage et des menus déroulants sur mesure coûte plus cher que des contrôles natifs et demande de porter soi-même l'accessibilité et la cohérence d'état entre la barre et la fiche, au profit d'une expérience de réservation premium, lisible et fidèle à la marque."
      }
    ],
    "challenges": [
      {
      ...
      {
        "problem": "Projeter une image premium crédible sur des connexions mobiles contraintes, pour une clientèle exigeante, tout en captant la réservation.",
        "solution": "Design system clair, chaleureux et honnête, photo-forward, avec un seul accent ocre doux (#B8863B) sur un monochrome espresso / crème (Fraunces en titrage, Hanken Grotesk en texte, navbar adaptative), servi en SSR par TanStack Start, avec photos optimisées par catégorie de studio et mise en page mobile-first. La barre de réservation type hôtel placée dans le hero sert de moteur de confiance et de conversion : elle reprend des codes que la clientèle connaît et mène directement à la fiche avec les dates pré-remplies."
      },
      {
        "problem": "Ajouter une couche d'animation premium et un hero responsive sans risquer l'écran blanc en SSR ni dégrader l'expérience sur mobile et sous prefers-reduced-motion.",
        "solution": "Un module de primitives GSAP (SplitText pour révéler les titres ligne par ligne derrière un masque, Reveal, Stagger, ImageReveal en clip-path) enregistré une seule fois, posé sur le hook useGSAP qui n'évalue le mouvement qu'après l'hydratation. Chaque animation part d'un état de base visible (fromTo, immediateRender:false) et n'agit que sur transform, opacity ou clip-path. Le hero s'adapte au point de rupture (panneau photo contenu à côté du texte en desktop, photo plein écran avec texte en surimpression en mobile), le slideshow enchaînant les vues en fondu avec un léger ken-burns. Les classes de révélation du markup interne ont été retirées pour éviter toute double animation, le build SSR passant sans erreur."
      }
    ],
    "results": [],
    "confidential": false,
